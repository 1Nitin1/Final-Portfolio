...
const ORRERY_HTML = `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
  ...
  <body>
    ...
    <script type="module">
      ...
      const themes = [
        {
          ...
          planetData: [
            { baseColor:[0.8,0.2,0.1],accentColor:[1,0.6,0.2] },
            { baseColor:[0.6,0.1,0.1],accentColor:[1,0.4,0.1] },
            { baseColor:[0.9,0.3,0],accentColor:[1,0.8,0.3] }
          ],
          ambientLightColor:0x401008,starLightColor:0xffcc88,
          directionalLights:{color1:0xff6600,color2:0xdd3300},
          metalMaterialColor:0x332222,ringColor:0xff8866
        },
        {
          name:'Veridian',
          starColors:{color1:0xccffee,color2:0x66ffcc},
          planetData:[
            {baseColor:[0.2,0.8,0.5],accentColor:[0.8,1,0.9]},
            {baseColor:[0.1,0.6,0.7],accentColor:[0.5,0.9,1]},
            {baseColor:[0.5,0.8,0.2],accentColor:[0.9,1,0.6]}
          ],
          ambientLightColor:0x0a3024,starLightColor:0xccffdd,
          directionalLights:{color1:0x33cc88,color2:0x4488cc},
          metalMaterialColor:0x779988,ringColor:0x88ffcc
        },
        {
          name:'Celestial',
          starColors:{color1:0xffe4b5,color2:0xff8844},
          planetData:[
            {baseColor:[1,0.4,0.4],accentColor:[1,0.8,0.2]},
            {baseColor:[0.3,0.8,0.3],accentColor:[0.6,1,0.8]},
            {baseColor:[0.3,0.4,1],accentColor:[0.8,0.6,1]}
          ],
          ambientLightColor:0x1a2440,starLightColor:0xffe4b5,
          directionalLights:{color1:0x4488ff,color2:0x8844ff},
          metalMaterialColor:0x4a6080,ringColor:0x88ccff
        }
      ];
      ...
      function createOrrery(){
        ...
        planetBaseData.forEach((data,i)=>{
          ...
          const planetMaterial=new THREE.ShaderMaterial({
            uniforms:{
              time:{value:0},
              baseColor:{value:new THREE.Vector3(...themes[currentThemeIndex].planetData[i].baseColor)},
      ...
      function createResonanceLinks(){
        resonanceLinks.forEach((entry)=>{
          if (entry.line?.geometry) {
            entry.line.geometry.dispose();
          }
          if (entry.line?.material) {
            entry.line.material.dispose();
          }
          if (entry.line?.parent) {
            entry.line.parent.remove(entry.line);
          }
        });
        resonanceLinks = [];

        if (planets.length < 3 || !star) {
          return;
        }

        const linkPairs = [
          { from: star, to: planets[0].body },
          { from: planets[0].body, to: planets[1].body },
          { from: planets[1].body, to: planets[2].body },
        ];
      ...
      function setResonanceLinksVisible(visible){
        resonanceLinks.forEach((entry)=>{
          if (!entry.line) {
            return;
          }
          ...
          if (entry.line.material) {
            entry.line.material.opacity = visible ? 0.82 : 0;
          }
        });
      }

      function updateResonanceLinks(){
        const fromPos = new THREE.Vector3();
        const toPos = new THREE.Vector3();

        resonanceLinks.forEach((entry)=>{
          if (!entry.line || !entry.from || !entry.to) {
            return;
          }

          entry.from.getWorldPosition(fromPos);
          entry.to.getWorldPosition(toPos);

          const positionAttr = entry.line.geometry.getAttribute('position');
          positionAttr.setXYZ(0, fromPos.x, fromPos.y, fromPos.z);
          positionAttr.setXYZ(1, toPos.x, toPos.y, toPos.z);
      ...
      function activateResonance(){
        ...
        const activationEffect={
          ...
          update:(elapsed)=>{
            ...
            setResonanceLinksVisible(false);
...
export function ResonanceOrrery() {
  return (
    <iframe
      title="Resonance Orrery"
      className="contact-orrery-iframe"
      srcDoc={ORRERY_HTML}
